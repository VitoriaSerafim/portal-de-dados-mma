...
const Dashboard = () => {
  ...
  function verify() {
    ...
    let element7 = document.getElementById(
      "nome-popular-checkbox"
    ) as HTMLInputElement;
    ...
    } else if (element7.checked) {
      setColuna1("Quantidade de especies sem nome popular");
      setColuna2(undefined)
      fetch("/sem-nome-popular")
        .then((res) => res.json())
        .then((data) => {
          setdata(data);
        });
    ...
    <>
      ...
      >
        ...
        >
          ...
          <table className="table-auto bg-gray-300 accent-pink-500 w-2/4 ml-96 mt-12 rounded-md m-12">
            <>
              <thead>
                <tr>
                  <th>{coluna1}</th>
                  {coluna2 != undefined ? <th>{coluna2}</th> : <></>}
                  {coluna1 === "Grupo Taxonômico" ? <th>{coluna3}</th> : ""}
                </tr>
              </thead>
              {data.map((tipo, index) => (
                <tbody>
                  <tr>
                    ...
                    {coluna1 == "Quantidade de especies sem nome popular" ? (
                      <>
          ...
          {coluna1 === "Situação" ? (
            <p style={{ marginLeft: "20vw", textAlign: "left" }}>
              {" "}
              Legenda: <br />
              Em Perigo (EN) <br />
              Vulnerável (VU)
              <br />
              Criticamente em Perigo - provavelmente extinto (CR)(PEX)
              <br />
              Extinta (EX)
              <br />
